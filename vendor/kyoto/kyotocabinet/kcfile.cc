...
bool File::open(const std::string& path, uint32_t mode, int64_t msiz) {
  ...
  void* map = NULL;
  if (msiz > 0) {
    const char *pmem_addr_str = getenv("PMEM_START_ADDR");
    uint64_t pmem_addr = strtoul(pmem_addr_str, NULL, 16);
    uint64_t orig_addr = pmem_addr;

    /* Suyash: Keep trying different PM ranges until successful */
    while (NULL == map || map == (void*)(-1)) {
      map = ::mmap((void*)pmem_addr, msiz, mprot,
                   MAP_SHARED | MAP_FIXED_NOREPLACE, fd, 0);
      pmem_addr += orig_addr/16;
    }
    if (map == MAP_FAILED) {
      perror("mmap");
...
bool File::synchronize(bool hard) {
  ...
  if (hard && core->msiz > 0) {
    int64_t msiz = core->msiz;
    if (msiz > core->psiz) msiz = core->psiz;
    if (msiz > 0 && snapshot(core->map, msiz, MS_SYNC) != 0) {
...
bool File::refresh() {
  ...
  int64_t msiz = core->msiz;
  if (msiz > core->psiz) msiz = core->psiz;
  if (msiz > 0 && snapshot(core->map, msiz, MS_INVALIDATE) != 0) {
    seterrmsg(core, "msync failed");
    err = true;
  }
  return !err;
...
bool File::end_transaction(bool commit) {
  ...
  if (core->trhard) {
    ...
    /* Suyash: Disable writes to WAL when running in CXL MODE */
    if (!::getenv("CXL_MODE_ENABLED")) {
      if (::fsync(core->fd) != 0) {
        seterrmsg(core, "fsync failed");
        err = true;
      }
...
static bool walwrite(FileCore *core, int64_t off, size_t size, int64_t base) {
  ...
  *(wp++) = WALMSGMAGIC;
  ...
  /* Suyash: Disable writes and fsync to WAL when running in CXL mode */
  if (!::getenv("CXL_MODE_ENABLED")) {
