    ...
    class ThreadTran : public kc::Thread {
     public:
      ...
      void run() {
        ...
        size_t tran_rnd = -1;
        const char *tran_rnd_str = getenv("TRAN_RND");
        if (tran_rnd_str != nullptr) {
          printf("got %s\n", tran_rnd_str);
          tran_rnd = strtoul(tran_rnd_str, nullptr, 10);
        }
        if (tran_rnd == -1) {
          fprintf(stderr, "Use TRAN_RND=<uint> to set commit probability\n");
          ::exit(1);          
        }
        printf("Using tran_rnd = %lu\n", tran_rnd);
