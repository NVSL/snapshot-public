...
public:
  ...
  const Value &get(const std::string &key) const {
    ...
    for (const auto &e : buckets[index]) {
      if (e.first == key) return values[e.second];
    }
  ...
  void put(const std::string &key, const Value &val) {
    ...
    for (const auto &e : buckets[index]) {
      if (e.first == key) {
        pmem::obj::transaction::run(pop, [&] { values[e.second] = val; });
    ...
    pmem::obj::transaction::run(pop, [&] {
      ...
      auto tmp_pair = std::make_pair(key, values.size() - 1);
      buckets[index].emplace_back(tmp_pair);

      // buckets[index].push_back(key, values.size() - 1);
    });
  }
};
