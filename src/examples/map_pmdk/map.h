...
struct map_ops {
  ...
  int (*insert)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key,
                PMEMoid value);
  int (*insert_new)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key,
                    size_t size, unsigned type_num,
                    void (*constructor)(PMEMobjpool *pop, void *ptr, void *arg),
                    void *arg);
  PMEMoid (*remove)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key);
  int (*remove_free)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key);
  int (*clear)(PMEMobjpool *pop, TOID(struct map) map);
  PMEMoid (*get)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key);
  int (*lookup)(PMEMobjpool *pop, TOID(struct map) map, uint64_t key);
  ...
  int (*cmd)(PMEMobjpool *pop, TOID(struct map) map, unsigned cmd,
             uint64_t arg);
};
...
int map_insert(struct map_ctx *mapc, TOID(struct map) map, uint64_t key,
               PMEMoid value);
int map_insert_new(struct map_ctx *mapc, TOID(struct map) map, uint64_t key,
                   size_t size, unsigned type_num,
                   void (*constructor)(PMEMobjpool *pop, void *ptr, void *arg),
                   void *arg);
...
int map_cmd(struct map_ctx *mapc, TOID(struct map) map, unsigned cmd,
            uint64_t arg);
