...
#    LD_PRELOAD="${ROOT}"/vendor/preeny/src/desock.so ltrace -n2 -w10 -o /tmp/cxlbuf.ltrace "${REDIS_INST_ROOT_PREFIX}"/redis-server "${REDIS_INST_ROOT_PREFIX}"/../redis.conf
...
execute() {
    ...
    printf "$(cat ~/git/libpuddles-scripts/traces/redis/$wrkld-load-1.0 ~/git/libpuddles-scripts/traces/redis/$wrkld-run-1.0)\nshutdown\n"  \
