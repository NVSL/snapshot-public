...
using namespace nvsl::libvramfs;

typedef std::string vramfs_path_t;
typedef int vramfs_fd_t;
...
std::unordered_map<vramfs_path_t, vramfs_fd_t> path_to_fd;
...
off_t nvsl::libvramfs::lseek(int fd, off_t offset, int whence) {
  off_t new_off = 0;
...
int nvsl::libvramfs::open(const char *pathname, int flags) {
...
void *nvsl::libvramfs::mmap(void *addr, size_t length, int prot, int flags,
                            int fd, off_t offset) {
  void *result;
